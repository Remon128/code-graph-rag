...
def main():
    ...
    # Header
    st.markdown('<div class="main-header">🤖 Code Graph RAG</div>', unsafe_allow_html=True)
    st.markdown("### AI-Powered Codebase Analysis and Modification")
    ...
    with st.sidebar:
        ...
        if is_healthy:
            ...
            if health_data:
                st.json({
                    "Memgraph": "Connected" if health_data.get("memgraph_connected") else "Disconnected",
                    "Agent": "Initialized" if health_data.get("agent_initialized") else "Not Initialized"
                })
        else:
            st.markdown(
    ...
    with st.form(key="chat_form", clear_on_submit=True):
        ...
        with col1:
            user_input = st.text_area(
                "Your message:",
                placeholder="Ask a question about your codebase or request modifications...",
                height=100,
    ...
    with st.expander("💡 Example Queries"):
        examples = [
            "Show me the main entry points in this codebase",
            "Find all functions that handle user authentication",
            "Explain how the configuration system works",
            "Refactor the logging setup to use structured logging",
            "Add type hints to all functions in main.py",
            "Create a new module for database utilities",
        ]
        ...
        for example in examples:
            if st.button(example, key=f"example_{examples.index(example)}"):
                ...
                with st.spinner("🤔 Agent is thinking..."):
                    result = send_message(example, st.session_state.session_id)
